...
export default function App() {
  ...
  // NEW: Track whether the user has toggled manual input adjustment
  ...
  // Automatic Device Geolocation Resolution Hook
  useEffect(() => {
    if (!isReporting) return;
    if (isManualLocation) return; // Prevent overwriting if user opted for manual entry
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            // Reverse Geocoding with MapTiler Cloud Engine using live coordinates
        ...
        (error) => {
          ...
          setCurrentCoords([3.4064, 6.4654]); // Safe default center coordinate if blocked
        },
  ...
  // NEW: Handler to activate manual adjustment mode
  const handleAdjustLocation = () => {
    setIsManualLocation(true);
    // If it's a loading message or error status, clear it out for clean typing
    if (newLocationName === 'Fetching live location...' || newLocationName === 'Your live location cannot be found' || newLocationName === 'Locating your position...') {
      setNewLocationName('');
    }
    // Timeout gives React a millisecond to register the disabled switch before focusing
    setTimeout(() => {
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    let targetCoordinates: [number, number] = currentCoords || [3.4064, 6.4654];

    // Optional: Forward geocode if they typed something manually
    if (isManualLocation && newLocationName.trim().length > 0) {
      try {
        const response = await fetch(
          `https://api.maptiler.com/geocoding/${encodeURIComponent(newLocationName)}.json?key=${MAPTILER_KEY}`
        );
        const data = await response.json();
        if (data.features && data.features.length > 0) {
          targetCoordinates = data.features[0].center;
        }
      } catch (err) {
        console.error("Forward geocoding failed, falling back to default/current map center coordinates:", err);
      }
    }
              ...
              <section className="form-section">
                ...
                <div className="location-box">
                  ...
                  <div className="location-details">
                    ...
                    />
                    <p className="meta">
                      {isManualLocation ? 'Entered manually' : 'Captured automatically'}
                    </p>
                  </div>
                </div>
                {/* MODIFIED: Runs the helper toggle function now */}
                <button type="button" className="adjust-location-btn" onClick={handleAdjustLocation}>
                  Not your location? <strong>Adjust</strong>
                </button>
              </section>

              <footer className="form-footer">
                <button type="submit" className="submit-btn" disabled={isSubmitting || (!isManualLocation && newLocationName === 'Locating your position...')}>
                  {isSubmitting ? 'Verifying live location...' : 'Continue'}
                </button>
              </footer>
            </form>
          </div>
