...
export default function App() {
  ...
  // 2. DYNAMIC MAP DRAG LISTENER FOR THE ADJUST SCREEN PIN
  useEffect(() => {
    ...
    const handleMapMove = async () => {
      ...
      if (center) {
        ...
        // Dynamic continuous check for pin alignment address text
  ...
  const handleCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      // Capture map center location instantly upon camera trigger snapshot
      if (mapRef.current) {
        const center = mapRef.current.getCenter();
          ...
          {!isReporting && !selectedReport && (
            <>
              ...
              <button 
                ...
                onClick={async () => {
                  // Lock current visible center instantly
                  if (mapRef.current) {
                    const center = mapRef.current.getCenter();
                    setCurrentCoords([center.lng, center.lat]);
              ...
              <button className={`report-trigger-btn ${currentTab === 'profile' ? 'active' : ''}`} onClick={() => setCurrentTab('profile')} style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <div style={{ width: '24px', height: '24px', borderRadius: '50%', backgroundColor: currentTab === 'profile' ? '#003366' : '#9CA3AF', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifycenter: 'center', fontSize: '10px', fontWeight: 'bold', marginBottom: '2px', display: 'flex', justifyContent: 'center' }}>
                  {getUserInitials(currentUser)}
                </div>
