...
export default function App() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setDisplayedLocation("Lagos, Nigeria");
          setCurrentCoords([3.3792, 6.5244]); // Default to centered Lagos coordinates
        },
  ...
  useEffect(() => {
    ...
    // Precise boundaries wrapping all of Lagos State
    const lagosBounds: [ [number, number], [number, number] ] = [
      [2.6924, 6.2201], // Southwest edge
      [4.2505, 6.7020]  // Northeast edge
    ];

    mapRef.current = new maplibregl.Map({
      container: mapContainerRef.current,
      style: `https://api.maptiler.com/maps/streets-v2/style.json?key=${MAPTILER_KEY}`,
      center: [3.3792, 6.5244], // Center on Mainland/Island nexus for clear visibility
      zoom: 10,
