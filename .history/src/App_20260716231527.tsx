...
export default function App() {
  ...
  // Real-Time Dynamic Location and Coordinates State
  ...
  // Automatic Geolocation Capture Trigger
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            // Reverse Geocoding with MapTiler Cloud Engine
        ...
        (error) => {
          ...
          setNewLocationName('Admiralty Way, Lekki Phase 1 (Fallback)');
          setCurrentCoords([3.4841, 6.4281]); // Safe Fallback coordinates if location permission is denied
        },
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    // Skip async forward geocoding query; use immediate captured coords or default Lagos center
    ...
    const newReport: FloodReport = {
      ...
      // Fallback placeholder image if no pictures are provided by choice
        ...
        <div className='navBar' style={{ pointerEvents: 'auto' }}>
          {!isReporting && !selectedReport && (
            <>
              ...
              {/* Direct access button bypasses mandatory quick camera trigger */}
              <button 
