...
export default function App() {
  ...
  // Helper to extract initials from Username or Email
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#1F2937', margin: '0 0 4px 0' }}>Flood Watch</h1>
            <p style={{ fontSize: '14px', color: '#6B7280', margin: 0 }}>
              {authScreen === 'signup' ? 'Create an account to start reporting' : 'Log in to access safe navigation maps'}
            </p>
          </div>

          {authError && (
            <div style={{ backgroundColor: '#FEF2F2', border: '1px solid #FCA5A5', color: '#B91C1C', padding: '12px', borderRadius: '12px', fontSize: '13px', marginBottom: '16px', fontWeight: 500 }}>
              {authError}
            </div>
          )}
          ...
          <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {authScreen === 'signup' && (
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#4B5563', marginBottom: '6px' }}>Username</label>
                <input 
                  type="text" 
                  placeholder="e.g. janesmith"
                  value={authUsername}
                  onChange={(e) => setAuthUsername(e.target.value)}
                  style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid #E5E7EB', fontSize: '15px', boxSizing: 'border-box', outline: 'none' }}
                />
              </div>
            )}
            ...
            {authScreen === 'signup' && (
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, color: '#4B5563', marginBottom: '6px' }}>Confirm Password</label>
                <input 
                  type="password" 
                  placeholder="••••••••"
          ...
          {!isReporting && !selectedReport && (
            <>
              ...
              <button 
                ...
                style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}
              >
                {/* Dynamically replacement of static profileIcon with custom user initials */}
                <div style={{
                  width: '24px',
                  height: '24px',
                  borderRadius: '50%',
