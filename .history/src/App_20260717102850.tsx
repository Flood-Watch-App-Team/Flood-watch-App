...
export default function App() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          // Fly the map to user's real coordinate location
          if (mapRef.current) {
            mapRef.current.flyTo({ center: [longitude, latitude], zoom: 14, essential: true });
          }

          try {
            // Reverse geocode to convert coords to a readable address
  ...
  // 2. SEARCH INTERFACE TRIGGER: Geocode search address, update name, and center map camera
  const handleMainSearchSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.features && data.features.length > 0) {
        ...
        // Clear input to showcase active area placeholder
        setMainSearchQuery('');
      } else {
